...
import type { DiagnosticoInput, DiagnosticoResult, ApiResponse } from "@/types";
...
interface UseDiagnosisReturn {
  ...
  analyze: (input: DiagnosticoInput) => Promise<DiagnosticoResult | null>;
  reset: () => void;
}

const PROGRESS_STEPS = [
  { at: 10, text: "Subiendo foto al servidor seguro..." },
  { at: 30, text: "Consultando catálogo SAG / INIA..." },
  { at: 50, text: "Analizando síntomas visuales..." },
  { at: 70, text: "Motor Surco IA activo..." },
...
export function useDiagnosis(): UseDiagnosisReturn {
  ...
  const analyze = useCallback(
    async (input: DiagnosticoInput): Promise<DiagnosticoResult | null> => {
      setAnalyzing(true);
      setError(null);
      setProgress(0);
      setProgressText("Iniciando análisis...");
      setResult(null);
